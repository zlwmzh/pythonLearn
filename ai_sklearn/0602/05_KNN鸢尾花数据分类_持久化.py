...
"""
# 使用joblib.dump这个API要求所给定的文件路径中的文件夹必须存在。后缀名无要求
filename = './models/knn.pk1'
dir_path = os.path.dirname(filename)
if not os.path.exists(dir_path):
    print("文件夹不存在，进行创建文件夹！！！")
    # os.mkdir() 只能创建一个文件夹 如./model/knn.pkl
    # os.makedirs() 可创建多个文件夹  如./model/knn/knn.pkl
    os.makedirs(dir_path)
joblib.dump(value=algo,filename='./models/knn.pk1')
